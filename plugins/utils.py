...
def label_style(data, default=None, filter=None, **kwargs):
    if data in filter:
...
def marker_set(data_list, shift=0):
    markers = [None, 's', '^', 'o', 'v']
    if len(markers) < len(data_list):
        raise BaseException("Please add more colors")
    else:
        return {data: markers[i + shift] for i, data in enumerate(data_list)}


def color_set(data_list):
    colors = ['b', 'r', 'g', 'darkorange', 'fuchsia', 'gold', 'yellowgreen',
              'sienna', 'royalblue', 'blueviolet', 'darkcyan', 'brown']
    if len(colors) < len(data_list):
        raise BaseException("Please add more colors")
    else:
        return {data: colors[i] for i, data in enumerate(data_list)}
...
def CheckOverlap(numpy_x, numpy_y, show='Yes'):
    x = numpy_x
    y = numpy_y
    length = len(x)
    i = 0
...
def find_bd(x_arr, current_arr, criteria, show='Yes'):
    x_arr, current_arr = CheckOverlap(x_arr, current_arr, show)
    xc = None
    for j, current in enumerate(current_arr):
        if current >= criteria:
            x1 = np.log10(x_arr[j - 1])
            x2 = np.log10(x_arr[j])
            i1 = np.log10(current_arr[j - 1])
            i2 = np.log10(current_arr[j])
            slope = (i2 - i1) / (x2 - x1)
            xc = x1 + (np.log10(criteria) - i1) / slope
            break
        elif current < 0:
            break
    if xc is not None:
...
def parse_csv(csv_path, key, Cname, X, floatKey, show='Yes'):
    ...
    :param show: print [utils.parse_csv]  Read file: *.csv
    :return: 取決於 key。
    """
    result = dict()
